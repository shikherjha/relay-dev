...
| **Last updated** | 2026-06-15 (Session 14 — matching retrieve→rerank + Bedrock /match-rank + Genie cleanup) |
...
| 2026-06-15 | **Windows host→container = `127.0.0.1`, never `localhost`** (relay-web `.env` + `api.ts` fallback) | `localhost` resolves to IPv6 `::1` which Docker Desktop black-holes → 30s `AbortError` hangs → empty Relay pages; IPv4 always works | ✅ Accepted |
| 2026-06-15 | **Rescue decay clock only for local TTL listings; shipped/national = "fixed price"** | `ttl_seconds` is null on national relists; the old `?? 3600` fallback rendered a bogus 60-min timer; decay pricing only applies to pickup-anchored local rescues | ✅ Accepted |
| 2026-06-15 | **Real PolygonScan links require live action in `USE_REAL_LEDGER` mode** (seed history stays unlinked) | Seeded events use fake hashes never sent to a chain; flipping the flag doesn't retro-anchor; a real link only comes from a genuine Amoy tx written during a live grade/resell/checkout | ✅ Accepted |
| 2026-06-15 | **Return grading must tolerate ML/Bedrock failure** (try real ML → fall back to local mock grade in `grade_and_store`) | Returns path 500'd when relay-ml/Bedrock was down/misconfigured, unlike the resale path which already falls back; a return must never break on a transient ML fault | ✅ Accepted |
| 2026-06-15 | **5xx responses must carry CORS headers** (catch-all middleware inner to `CORSMiddleware`) | Starlette's `ServerErrorMiddleware` sits outside CORS → unhandled 500s lost their ACAO header and surfaced in the browser as a misleading CORS / "Failed to fetch", hiding the real error | ✅ Accepted |
| 2026-06-15 | **Rescue feed sorts newest-return-first** (`returned_at` desc, nationals last), not by `expires_at`/distance | Expiry is pickup-anchored (`created_at + TTL`) so it's a bad recency proxy — a just-returned unit could rank below one with more TTL left; surface the unit's latest `ReturnEvent` time instead | ✅ Accepted |
| 2026-06-15 | **Rescue credits are immediately spendable** (`unlock_at=None`); p2p second-life buys keep the 14-day lock | The reward must visibly land in the wallet the moment you rescue (header coin = spendable); the 14-day lock stays for resale purchases where a cool-off is reasonable | ✅ Accepted |
| 2026-06-15 | **Resale `grade_and_price` must tolerate full ML outage** (two-stage fallback → deterministic mock grade) | Same class as the returns 500: the prior fallback re-called the real grade endpoints, which also throw when relay-ml/Bedrock is down → unhandled 500 on seller relist; a resell/relist must never break on an ML fault | ✅ Accepted |
| 2026-06-15 | **Rescue/Second-Life cards + order rows use real `image_url`** (S3), category image only as fallback | Cards rendered `categoryImage()` only → "shirt" (no key) fell through to the generic fashion/jeans photo; use the seeded S3 catalogue image so the right product shows | ✅ Accepted |
| 2026-06-15 | **Mock + live ledger event shapes must agree** (`explorer_url` added to the `ledgerVerify` mock event) | `LedgerData` intersects the mock event shape with `LedgerVerifyDTO`; the mock lacked `explorer_url` so the timeline's PolygonScan link failed `tsc` despite working on live data — keep mock/live DTOs in sync | ✅ Accepted |
| 2026-06-15 | **Genie matching is vertical-gated** (electronics vs fashion) + drops anti-correlated cosine hits | Pure pgvector cosine returned cross-vertical garbage (Macbook wish → Cotton Crew Tee); classify the wish's vertical from free text and gate candidates to it | ✅ Accepted |
| 2026-06-15 | **Wishes persisted via backend** (`GET /wishlist` source of truth + `DELETE /wishlist/{id}`) | "Your wishes" rendered only from a non-persisted store → vanished on reload; the DB must be the source of truth, deletes go server-side | ✅ Accepted |
| 2026-06-15 | **Rescue feed sorts purely by `returned_at` desc** (all scopes) | Most-recent return must be on top regardless of local/national (a just-returned MacBook was sinking below locals under the nationals-last rule) | ✅ Accepted |
| 2026-06-15 | **Matching = retrieve→rerank** (vertical-gated cosine recall → Bedrock `/match-rank` LLM rerank → deterministic taxonomy fallback + hard category veto, floor 0.5) | Cosine alone returned earphones for "macbook" / tee for "hoodie"; industry pattern (category-aligned retrieval + LLM rerank) fixes intent matching; taxonomy veto guarantees no cross-category even if the LLM/relay-ml is unavailable | ✅ Accepted |
| 2026-06-15 | **New relay-ml endpoint `POST /match-rank`** (Nova Lite, text-only) as the AI matching layer | "Just cosine" is too coarse; an LLM judges product-type/intent relevance cheaply; fails loud (503) so relay-api falls back to taxonomy | ✅ Accepted |
| 2026-06-15 | **Genie "Live matches" section removed** | Redundant — matches are reached via "View match" inside "Your wishes"; one matches surface, not two | ✅ Accepted |
...
| 2026-06-15 | Session 9: **Relay UI bug fixes (HMR, no rebuild)** — rescue feed newest-first + clickable cards + correct/gated decay clock (`DecayClock` now `expiresAt`-driven); ledger page doubles as product page (price/AI range/description/add-to-cart/rescue-only timer via live feed cross-reference); Genie "Your wishes" section (persisted `myWishes`, watching→granted correlation). **Blockchain explainer** (mock vs real, why seed history is unlinked, how to demo a real PolygonScan link). **"Empty Relay" outage root-caused = 3 stacked infra issues**: IPv6 `localhost` black-hole (→ `127.0.0.1`), wedged Docker Desktop vpnkit port proxy (→ `wsl --terminate docker-desktop` + recreate), stray `POSTGRES_PASSWORD` shell var → DB auth fail (→ clear var + recreate; data preserved). All endpoints 200 from host. Frontend changes uncommitted | Commit/push FE fixes; deploy + demo video + submission |
| 2026-06-15 | Session 10: **Bugfix — return media upload 500 masked as CORS.** `POST /returns/{id}/media` 500'd because `grade_and_store` called the ML client synchronously with no error handling (relay-ml/Bedrock down/misconfigured → uncaught exception); the 500 lost its CORS header (ServerErrorMiddleware sits outside CORS) so the browser showed "Failed to fetch". Fixed in relay-api: (1) `services/grading.py` — `grade_and_store` now tries real ML then falls back to `MockMLClient` local grade (mirrors resale path); (2) `main.py` — added a catch-all `BaseHTTPMiddleware` inner to `CORSMiddleware` so 5xx keep CORS headers and real errors surface. Verified: imports clean, 500 carries ACAO + preflight 200, `pytest` 2 passed. Changes uncommitted | Commit/push FE+BE fixes; deploy + demo video + submission |
| 2026-06-15 | Session 11: **Rescue/wallet/seller bug sweep (5 fixes).** (1) Rescue cards now use real S3 `image_url` via `productImage()` (was `categoryImage()` only → shirt fell back to jeans). (2) Feed sorts newest-return-first via new `returned_at` (was `expires_at`/distance). (3) Wallet order rows clickable → product page. (4) Rescue now awards immediately-spendable green credits (direct claim awarded none; checkout locked them 14d). (5) Seller "Grade & relist" 500 fixed — `grade_and_price` two-stage fallback to deterministic mock grade. relay-api: `clients/ml_client.py`, `schemas/rescue.py`, `routers/rescue.py`, `services/orders.py`; relay-web: `lib/relay-api.ts`, `routes/rescue.tsx`, `components/relay/BuyerActivity.tsx`. `pytest` 2/2. Uncommitted | Commit/push FE+BE; deploy + demo video + submission |
| 2026-06-15 | Session 12: **Fixed deferred `tsc` error** in `relay-web/src/routes/ledger.$unitId.tsx` — added `explorer_url` to the `ledgerVerify` mock event (`lib/mock-extra.ts`) so the intersected `LedgerData` event type matches `LedgerVerifyDTO`. `npx tsc --noEmit` exit 0 → unblocks `npm run build`. FE-only (HMR). Uncommitted | Commit/push FE+BE; deploy + demo video + submission |
| 2026-06-15 | Session 13: **Genie/rescue correctness (4 fixes).** (1) Matching vertical-gated (`classify_vertical` + SQL `Product.vertical` gate, drop `sim<=0`) → no cross-vertical matches; match cards get real `image_url`. (2) Wish persistence: `GET /wishlist` (source of truth) + `DELETE /wishlist/{id}`; Genie loads from backend, deletes server-side. (3) Rescue feed sorts purely by `returned_at` desc (most-recent return on top, all scopes). (4) confirmed Session-11 image/order-link fixes. relay-api: `services/matching.py`, `schemas/wishlist.py`, `routers/wishlist.py`, `routers/rescue.py`; relay-web: `lib/api.ts` (`del()`), `lib/relay-api.ts`, `routes/genie.tsx`, `routes/rescue.tsx`. `pytest` 2/2, `tsc` exit 0. Uncommitted | Within-vertical matching still weak → Session 14 (taxonomy + AI rerank); remove redundant Genie "Live matches"; commit/push; deploy + demo video |
| 2026-06-15 | Session 14: **Matching retrieve→rerank + AI layer + Genie cleanup.** Two-stage matcher: vertical-gated cosine recall → relay-ml **`POST /match-rank`** (Bedrock Nova Lite) LLM rerank → deterministic `core/taxonomy.py` fallback with hard category veto (floor 0.5). Fixes earphones-for-macbook / tee-for-hoodie / backpack-for-tee (all now filtered → "Watching for a match"). `match_rank()` added to ML client (Protocol/Mock/HTTP); `match_for_user(ml=)` threaded. Removed redundant Genie "Live matches" section. relay-api: `core/taxonomy.py` (new), `services/matching.py`, `clients/ml_client.py`, `routers/wishlist.py`; relay-ml: `pipelines/match_rank.py` (new), `routers/match.py` (new), `main.py`; relay-web: `routes/genie.tsx`. `pytest` 2/2, relay-ml `py_compile` ok, `tsc` exit 0. **Restart relay-ml + relay-api** (rebuild). Uncommitted | Commit/push all 3 repos; deploy + demo video + submission |
